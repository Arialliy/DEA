...
9. Event-Atomic Rooted Geodesic Field 为 **NO-GO**。虽然正边权 geodesic ball 保证单个 support 连通、组件分数广播可令单个 atom 整体过阈值，但 `strict local maximum → root → ball` 机械上仍是 dense proposals；重复 root、support overlap/touch 与 bridge 必须再靠 NMS、hard-core/set packing、watershed/forest 或 score arbitration。三数据集 eval256 还有 135 个仅靠对角保持 8 连通的 GT components，四邻域版本本身即不完备；改成八邻域也不能消除 proposal/arbitration 问题。由于修复会重新引入已禁止的多阶段组件选择，该路线在 GPU 前终止，实验性 M0 代码已删除而不留死分支。
