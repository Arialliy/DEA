...
> 文档状态：**2026-07-12，Gate F−1a Q0/Q1/Q2 sensitivity 已完成并判 NO-GO；targeted/full unique-logit sweep 未获授权，risk-control 主线不重开。**

基于当前代码、冻结 E−1c bundle 和已完成的 Gate F0，结论如下：

> **现在不应继续设计 MSHNet 网络模块或复合 loss。54 点分位网格确实不是 event-complete，但 full exact curve 只能审计经验网格离散化，不能突破 Gate F0 对 generic distribution-free HB-LTT / unit-bound CRC 所证明的 rare-event 信息下界。严格嵌套的 Q0/Q1/Q2 sensitivity 已完成：四个预算均未达到跨数据集触发门，因此停止 targeted/full unique-logit sweep。**
...
\[
\boxed{
\text{Q0/Q1/Q2 nested-grid sensitivity：NO-GO}
\rightarrow
\text{targeted/full exact interval：停止}
\rightarrow
\text{表示级失败分解}
\rightarrow
\text{Gate G 方向判定}
...
该结果只属于 `alternative-grid sensitivity`，不会追溯修改冻结的 E−1c formal decision。Generic LTT/CRC 已由 Gate F0 判定 NO-GO，不再重复生成全候选 outcome cache。
...
|---|---:|
| 总像素分数 | 66,060,288 |
| \(\sum_j|\mathrm{unique}(z_j)|\)：image-local distinct float32 score groups | 65,599,473 |
| distinct-event / pixel 比例 | 99.302433% |
| naive matcher-event evaluations \(2\sum_jm_j\) | 131,198,946 |

以上数字已由下列 immutable bundle 连同 checkpoint、registry、source 与 artifact hashes 固化：

```text
$HOME/DEA/repro_runs/gate_f/nested_grid_sensitivity_v1
```
...
Q0 对冻结 E−1c 的 calibration curves、selections、逐图 counts、逐 target status 和两折 pooled aggregates 全部精确 replay。legacy 与 Hungarian 的 108 个 `dataset × seed × budget × grid-level` pooled pairs 在本 bundle 中全部一致，但实现与输出仍保持独立。

| FA/Mpix | Q0→Q2 zero-overshoot flip（双 matcher） | 通过 material-gain 的 datasets | Q1=Q2 threshold pairs | Gate |
|---:|---|---:|---:|---:|
| 1 | IRSTD-1K / 20260711，pass→fail | 0 | 4/9 | FAIL |
| 5 | IRSTD-1K / 20260713，pass→fail | 0 | 1/9 | FAIL |
| 10 | NUAA-SIRST / 20260712，fail→pass | 0 | 2/9 | FAIL |
| 20 | NUDT-SIRST / 20260713，pass→fail | 0 | 4/9 | FAIL |

Q0→Q2 selected threshold pair 分别在 8/9、9/9、7/9、8/9 dataset-seeds 上改变，说明原 grid 确实不是数值不敏感；但这些变化没有形成跨数据集、跨 seed 的同向机制。最明显的反例是 IRSTD-1K / 20260713 / FA=5：held-out Pd 从 0.5833 升至 0.7579，同时 FA 从 3.2425 升至 7.1526/Mpix，使原本可行的点变为超调。唯一 fail→pass 是 NUAA-SIRST / 20260712 / FA=10，仍是单 dataset-seed。
...
\text{Gate F−1a FAIL}
\Rightarrow
\text{targeted/full exact sweep 不授权}
}
\]

这一结果不是说 omitted events 不存在，而是说在预注册资源门下，没有证据支持继续为其构建 6560 万事件级 solver。
...
实际决定：四个预算全部 FAIL，Gate F−1a 已关闭。
...
> **保持 MSHNet 完全不变。Q0/Q1/Q2 已证明 54 点候选族存在局部数值敏感性，但没有跨数据集、跨 seed 的同向实质翻转，因此 exact-event 路线停止。它也不能突破 Gate F0 的 rare-event 样本下界；下一主线回到有真实覆盖率、且不退化为 topology/hard-negative/CFAR 的表示级机制。**
...
4. Q0/Q1/Q2 的孤立 flip 只写成 `alternative-grid sensitivity`；
5. Gate 未触发，停止 targeted/full exact curve；
